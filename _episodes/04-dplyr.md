...
In the previous episode we used the `readr` package to load the gapminder data into a tibble within R.  In this episode we'll use the `dplyr` package to manipulate the data we loaded, and calculate some summary statistics.  We'll also introduce the concept of "pipes".
...
> 
> Instead of saying what columns we _do_ want, we can tell R which columns we don't want by prefixing the column name with a `-`.  For example to select everything except year we would use `select(gapminder, -year)`.
> 
...
* [Data Wrangling Cheat sheet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf) - you can also access this from the help menu in RStudio (in newer versions of RStudio it has been replaced with "Data Transformation with dplyr")
